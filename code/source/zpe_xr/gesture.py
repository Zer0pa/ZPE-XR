...
PROTOTYPE_SCALES = (0.020, 0.020, 0.018, 0.018, 0.018, 0.018, 0.020, 0.020, 10.0)
...
def classify_gesture(
    ...
    labels = tuple(vocabulary or PROTOTYPES)
    if not labels:
        raise ValueError("vocabulary must not be empty")
    unknown = [label for label in labels if label not in PROTOTYPES]
    if unknown:
        raise ValueError(f"unknown gesture labels: {unknown}")
    ...
    for label in labels:
        ...
        score = 0.0
        for o, p, scale in zip(observed, prototype, PROTOTYPE_SCALES):
