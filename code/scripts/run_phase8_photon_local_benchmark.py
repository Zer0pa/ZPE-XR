#!/usr/bin/env python3
"""Run the Phase 08 same-machine Photon local benchmark."""
...
from zpe_xr.phase8_photon_benchmarks import benchmark_report, render_markdown
...
def main() -> int:
    ...
    synthetic_frames = generate_sequence(num_frames=args.frames, gesture=args.gesture, seed=args.seed)

    experiment = create_experiment(
        name=f"phase8-photon-local-{run_id}",
        tags=["phase-8", "photon", "local-incumbent"],
        parameters={
    ...
    write_json(artifact_dir / "phase8_photon_local_benchmark.json", summary)
    if "benchmark" in summary:
        (artifact_dir / "phase8_photon_local_benchmark.md").write_text(
            render_markdown(summary["benchmark"]),
            encoding="utf-8",
        )
        log_mapping(experiment, "phase8.photon_local", summary["benchmark"])
    ...
    for name in [
        ...
        "phase8_photon_local_benchmark.md",
    ...
    print(f"Phase 08 Photon local benchmark complete. STATUS={summary['execution_status']}")
